...
$(document).ready(function() {
    ...
    return {
        ...
        onSelect: function(fd, startDateText, endDateText) {
            if(isLeft) {
                startDateText = datepickerfindApartLeft.selectedDates[0];
                endDateText = datepickerfindApartLeft.selectedDates[1];
            }
            else {
                startDateText = datepickerfindApartRight.selectedDates[0];
                endDateText = datepickerfindApartRight.selectedDates[1];
            }
            $("#dateDropdownLeft_findApart").val(getValue(startDateText));
            $("#dateDropdownRight_findApart").val(getValue(endDateText));
        },
            ...
            if (!animationCompleted) {
               if (dp.$datepicker.find('h3').html()===undefined) { /*ONLY when button don't existis*/
                  ...
                  dp.$datepicker.find('button').click(function(event) {
                    $("#dateDropdownLeft_findApart").val("");
                    $("#dateDropdownRight_findApart").val("");
                    datepickerfindApartLeft.clear();
                    datepickerfindApartRight.clear();
                 });
               }
            }
         }
    ...
    let datepickerfindApartLeft = $('#dateDropdownLeft_findApart').datepicker(settings(true)).data('datepicker');
    let datepickerfindApartRight = $('#dateDropdownRight_findApart').datepicker(settings(false)).data('datepicker');
});